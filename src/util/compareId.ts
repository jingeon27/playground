type returnParamType<T extends Object> = { id: number | string } & { [key in keyof T]: T[key] }
...
export function compareId
    (compareValue?: number | string, isSame = true) {
    return <T extends Object>({ id }:returnParamType<T>)  =>
        isSame ? String(compareValue) === String(id) : String(compareValue) !== String(id)
}
